...
public class Sample {
    ...
    private static String[] SAMPLE_ARRAY = { "A", "あ", "㐀", "㐂", "㐂\uDB40\uDD00", "㐂豈", "一", "豈", "😀", "𠀀" };

    public static void main(final String[] args) {

        // String Sample
        System.out.println(String.class.getSimpleName());
        System.out.println(SAMPLE_STRING.length());// 14
        System.out.println(SAMPLE_STRING.chars().mapToObj(codePoint -> Character.valueOf((char) codePoint).toString()).collect(Collectors.joining(", ")));
        // 芦, 田, さ, ん, は, 芦, ?, ?, 屋, の, お, 嬢, 様, だ,
        SortedSet<String> set = new TreeSet<>();
        Collections.addAll(set, SAMPLE_ARRAY);
        System.out.println(set.stream().collect(Collectors.joining(", ")));
        // A, あ, 㐀, 㐂, 㐂󠄀, 㐂豈, 一, 😀, 𠀀, 豈,
        System.out.println(SAMPLE_STRING.subSequence(5, 9)/* 芦\uDB40\uDD01屋 */);
        System.out.println();
        ...
        set = new TreeSet<>(
                (final String s1, final String s2) -> new OneCharSequence(new UniCharSequence(s1)).compareTo(new OneCharSequence(new UniCharSequence(s2))));
        Collections.addAll(set, SAMPLE_ARRAY);
        System.out.println(set.stream().map(oneChar -> oneChar.toString()).collect(Collectors.joining(", ")));
        // A, あ, 㐀, 㐂, 㐂󠄀, 㐂豈, 一, 豈, 😀, 𠀀,
