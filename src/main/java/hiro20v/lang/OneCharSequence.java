...
public class OneCharSequence implements Comparable<OneCharSequence> {
	...
	public OneCharSequence toOneCharSequenceIgnoreVariation() {
		...
		final List<OneChar> oneCharListIgnoreVariation = this.oneCharList.stream().sequential().map(oneChar -> oneChar.toOneCharIgnoreVariation()).collect(Collectors.toList());
	 ...
	 * 2つの文字列が異なる場合、両方の文字列に対して有効なインデックスに位置する文字が異なるか、2つの文字列の長さが異なるか、 あるいはその両方が該当します。
	...
	Information getInformation() {
		...
		information.setName(OneCharSequence.class.getSimpleName());

		information.getProperties().put(String.class.getSimpleName(), "'" + this.toString() + "'");
		information.getProperties().put("Length", String.valueOf(this.length()));

		this.toCharList().forEach(oneChar -> {

			final Information childInformation = oneChar.getInformation();
